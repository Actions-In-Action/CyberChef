...
echo "the latest_tag of gchq/CyberChef is ${github_latest_tag}"
...
gitee_tag=`curl --silent -X GET --header 'Content-Type: application/json;charset=UTF-8' "https://gitee.com/api/v5/repos/${SECRETS_GITEE_USERNAME}/CyberChef/tags"`
if [ $? -ne 0 ]; then
...
unzip -o -q CyberChef_${github_latest_tag}.zip
if [ $? -ne 0 ]; then
...
git push --repo https://${SECRETS_GITEE_USERNAME}:${SECRETS_GITEE_PASSWORD}@gitee.com/${SECRETS_GITEE_USERNAME}/CyberChef.git > /dev/null 2>&1
if [ $? -ne 0 ]; then
...
git push --repo https://${SECRETS_GITEE_USERNAME}:${SECRETS_GITEE_PASSWORD}@gitee.com/${SECRETS_GITEE_USERNAME}/CyberChef.git --tags > /dev/null 2>&1
if [ $? -ne 0 ]; then
